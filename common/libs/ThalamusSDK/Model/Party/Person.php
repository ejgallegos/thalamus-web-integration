...
class Person extends ThalamusModel {
	...
	public function changePassword($oldPassword, String $newPassword, String $confirmNewPassword) {
	    ...
	    $r = ThalamusRequest::put ( "/person/passwordchange?oldPasswordRequired=$oldPasswordRequired", $params )->auth($session->getRealUser(), $session->getAccessToken())->execute ();
	...
	public function getSocialConnections() {
		return ThalamusRequest::get ( '/connect', array () )->execute ()->getThalamusModelList ( SocialConnection::classname (), 'socialConnections' );
	}
	...
	public function update(Array $profile = null, Array $optIns = null, Array $consumers = null) {
		...
		if ($profile != null) {
			...
			if (!empty($params['profile']['birthday']) && (ThalamusHelper::validateDate($params['profile']['birthday']) == false)) {
		...
		return ThalamusRequest::put ( '/person', $params )->execute ();
